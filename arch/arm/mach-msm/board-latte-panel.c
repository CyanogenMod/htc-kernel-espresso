...
#define LATTE_BR_DEF_USER	102
#define LATTE_BR_MIN_USER	30
#define LATTE_BR_MAX_USER	255
#define LATTE_BR_DEF_PANEL	122
#define LATTE_BR_MIN_PANEL	8
#define LATTE_BR_MAX_PANEL	255

static int
latte_panel_shrink(int brightness)
{
	if (brightness <= LATTE_BR_DEF_USER) {
		if (brightness <= LATTE_BR_MIN_USER)
			brightness = LATTE_BR_MIN_PANEL;
		else
                        brightness = (LATTE_BR_DEF_PANEL - LATTE_BR_MIN_PANEL) *
				(brightness - LATTE_BR_MIN_USER) /
				(LATTE_BR_DEF_USER - LATTE_BR_MIN_USER) +
				LATTE_BR_MIN_PANEL;
                } else
                        brightness = (LATTE_BR_MAX_PANEL - LATTE_BR_DEF_PANEL) *
				(brightness - LATTE_BR_DEF_USER) /
				(LATTE_BR_MAX_USER - LATTE_BR_DEF_USER) +
				LATTE_BR_DEF_PANEL;
