...
struct atmel_i2c_platform_data liberty_ts_atmel_data[] = {
	{
		...
		.config_T7 = {50, 15, 25},
		.config_T8 = {8, 0, 20, 10, 0, 0, 10, 10},
		.config_T9 = {139, 0, 0, 16, 10, 0, 16, 35, 2, 1, 0, 5, 2, 46, 2, 10, 20, 0, 0, 0, 0, 0, 1, 1, 20, 20, 153, 54, 153, 89},
			       ...
			       unsigned long flags)
{
	if ((flags & GPIOF_DRIVE_OUTPUT) && !vreg_h2w)
		vreg_h2w = vreg_get(0, LIBERTY_H2W_POWER_NAME);
...
static struct gpio_chip liberty_h2w_gpio_chip = {
	.start = LIBERTY_GPIO_H2W_POWER,
	.end = LIBERTY_GPIO_H2W_POWER,
...
void liberty_init_h2w_power_gpio(void)
{
	register_gpio_chip(&liberty_h2w_gpio_chip);
}
...
{
	if (system_rev > 1) {
			/* center: x: home: 17, menu: 82, back: 235, search 302, y: 510 */
		return sprintf(buf,
			__stringify(EV_KEY) ":" __stringify(KEY_HOME)	    ":17:510:35:55"
			":" __stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":82:510:40:55"
			":" __stringify(EV_KEY) ":" __stringify(KEY_BACK)   ":235:510:40:55"
			":" __stringify(EV_KEY) ":" __stringify(KEY_SEARCH) ":302:510:35:55"
			"\n");
	} else {
