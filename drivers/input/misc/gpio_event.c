...
#if (CONFIG_MACH_HEROC || CONFIG_MACH_LATTE)
...
#else
static unsigned long filter_time = 17;
static struct _filter_key_set key_set[] = {
	{
		.filter_key = { KEY_F17},/* [Espresso] Extra delay on Voice Dialer Key*/
		.key_jiff_stamp = 0,
	}
};
#endif
...
#if (CONFIG_MACH_HEROC || CONFIG_MACH_LATTE)
	if(machine_is_heroc() ||machine_is_latte()  ) {
...
#if (CONFIG_MACH_HEROC || CONFIG_MACH_LATTE)
void button_release(struct input_dev *dev,
				...
				for(loop_j = 0; loop_j < 2; loop_j++) {
					if(value == 0) {
						if(code == filter_key_data.filter_key_set[loop_i].filter_key[loop_j]){
							if((filter_key_data.filter_key_set[loop_i].force_release_key & (0x1 << loop_j)) == (0x1 << loop_j)){
								filter_key_data.filter_key_set[loop_i].force_release_key &= ~(0x1 << loop_j);
								printk(KERN_INFO "%s: key %x release key_jiff_stamp: %lu\n",
									__func__,code,filter_key_data.filter_key_set[loop_i].key_jiff_stamp);
							}
						}
					}
				}
		...
		for(loop_i = 0; loop_i < filter_key_data.set_num; loop_i++) {
			if(code == filter_key_data.filter_key_set[loop_i].filter_key[0] ||
					...
					if(value) {
						if(code == filter_key_data.filter_key_set[loop_i].filter_key[loop_j]){
							...
							if((filter_key_data.filter_key_set[loop_i].force_release_key & (0x1 << loop_j)) != (0x1 << loop_j))
							{
								...
								printk(KERN_INFO "%s: filter_key_data.filter_key_set[loop_i].key_jiff_stamp: %lu\n",
									__func__,filter_key_data.filter_key_set[loop_i].key_jiff_stamp);
							}
						#endif
							filter_key_data.filter_key_set[loop_i].force_release_key |= (0x1 << loop_j);
						}
						...
						printk(KERN_INFO "%s: release key jiff_stamp: %lu\n",
									__func__,filter_key_data.filter_key_set[loop_i].key_jiff_stamp);
					}
				}
				if(filter_key_data.filter_key_set[loop_i].key_jiff_stamp && value &&
					...
					for(loop_j = 0; loop_j < 2; loop_j++) {
						if(code == filter_key_data.filter_key_set[loop_i].filter_key[loop_j]) {
							printk(KERN_INFO "%s: in filter_time, discard: %d(%lu)\n", __func__, code, *key_pressed);
							ret = 0;
							break;
						}
					}
				}else{
					printk(KERN_INFO "%s: send: %d(%lu), press %d\n", __func__, code, *key_pressed,value);
				}
			}
		}
	#ifdef CONFIG_MACH_HEROC
